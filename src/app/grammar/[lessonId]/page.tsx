...
const GrammarLessonPage = () => {
  ...
  useEffect(() => {
    // This page is for everyone, so we don't redirect if not logged in.
    // However, progress will only be saved for logged-in users.
    if (user && firestore) {
  ...
  useEffect(() => {
    if (Object.keys(progress).length > 0 && user) {
      // Only check achievements if user is logged in
      checkAchievements(progress);
    }
  }, [progress, checkAchievements, user]);
